...
Distribution.prototype.getConfidenceEllipse = function(angle) {
  ...
  if(this.constructor === PoleDistribution) {
    return new this.constructor(vectors).rotateTo(this.mean.lng, this.mean.lat).vectors;
  } else if(this.constructor === DirectionDistribution) {
    return new this.constructor(vectors).rotateTo(this.mean.dec, this.mean.inc).vectors;
  }
...
Distribution.prototype.rotateVector = function(azimuth, plunge, vector) {
  ...
  var rotationMatrix = new Array(
    new Array(Math.sin(plunge) * Math.cos(azimuth), -Math.sin(azimuth), -Math.cos(plunge) * Math.cos(azimuth)),
    new Array(Math.sin(plunge) * Math.sin(azimuth), Math.cos(azimuth), -Math.cos(plunge) * Math.sin(azimuth)),
    new Array(Math.cos(plunge), 0, Math.sin(plunge))
  );
